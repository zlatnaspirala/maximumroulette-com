...
/* global THREE render PROGRAM OSCILLATOR group */
...
function TEXT3D (position) {
  ...
  ROOT.size = 30,
  ROOT.hover = 30,
  ROOT.curveSegments = 4,
  ROOT.bevelThickness = 2,
  ROOT.bevelSize = 1.5,
  ROOT.bevelSegments = 3,
  ROOT.bevelEnabled = true,
  ROOT.font = undefined,
  ROOT.fontName = "optimer",
  ...
  ROOT.material = new THREE.MultiMaterial( [
    new THREE.MeshPhongMaterial( { color: 0xff0600, shading: THREE.FlatShading } ), // front
    new THREE.MeshPhongMaterial( { color: 0x6f6f00, shading: THREE.SmoothShading } ) // side
  ] );
  ...
  ROOT.UPDATE_LIGHT_DINAMIC = true;
  ROOT.UPDATE_LIGHT_BY_X = true;
  ROOT.UPDATE_LIGHT_BY_Y = false;
  ROOT.UPDATE_LIGHT_BY_Z = false;
  ROOT.UPDATE_LIGHT_MARGIN_X = new OSCILLATOR( 0,1300,10);
...
function loadFont(textSettings_) {
  ...
  //console.log('fonts/' + textSettings_.fontName + '_' + textSettings_.fontWeight + '.typeface.js')
...
function createText(textSettings_) {
  ...
  var textGeo = new THREE.TextGeometry( textSettings_.text, {
  ...
  textSettings_.pointLight = new THREE.PointLight( 0xffffff, 10 ,  100);
  textSettings_.pointLight.position.set( 122 , 80 , 80);

  var sphere = new THREE.SphereGeometry( 2, 16, 8 );
  textSettings_.pointLight.add( new THREE.Mesh( sphere, new THREE.MeshBasicMaterial( { color: 0xff0040 } ) ) );
  textSettings_.group.add( textSettings_.pointLight  );
  //scene.add( textSettings_.pointLight );
