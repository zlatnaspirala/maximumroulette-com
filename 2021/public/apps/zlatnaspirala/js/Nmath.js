...
function DETECTBROWSER() {
  ...
  if (navFirefox && navandroid && TYPEOFANDROID == 0) {
    browser_name = "androind_firefox";
    HREFTXT = "You using mobile FireFox on android,GO TO PLAY NOW!";
  }

  //  FIREFOX za android T
  if (navFirefox && navandroid && TYPEOFANDROID == 1) {
    browser_name = "android_tab_firefox";
    HREFTXT = "You using mobile FireFox on android TABLET,GO TO PLAY NOW!";
  }

  // OPERA ZA ANDROID
  if (navOpera && navandroid) {
    browser_name = "android_opera";
    HREFTXT = "You using opera mobile browser on android ,GO TO PLAY NOW!";
  } // provera

  // OPERA ZA ANDROID TABLET
  if (navOpera && navandroid && operatablet) {
    browser_name = "android_tab_opera";
    HREFTXT =
      "You using opera mobile browser on android TABLET!, GO TO PLAY NOW!";
  } // provera
  ...
  if (navSafari) {
    ...
    if (Iphonesafari) {
      browser_name = "iphone_safari";
      HREFTXT = "You using safari mobile browser on IPHONE device, Go to play!";
    } // OK
    else if (navIpad) {
      browser_name = "ipad_safari";
      HREFTXT = "You using mobile safari on IPAD +chrome suport !";
    } //OK
    else if (navandroid) {
      browser_name = "android_nativ?";
      HREFTXT = "> Android native navigator: NO SUPORT for HTML5!";
    }
  }

  // TEST CHROME
  if (navChrome && navSafari && navMozilla) {
    browser_name = "android_tab_chrome";
    HREFTXT =
      "You using mobile chrome browser on android tablet, GO TO PLAY NOW!";
  }

  if (navChrome && TYPEOFANDROID == 0) {
    browser_name = "desktop_chrome";
    HREFTXT = "You using  chrome browser on desktop, GO TO PLAY NOW!";
  }

  if (navMozilla && NOMOBILE == 1 && gecko && navFirefox) {
    browser_name = "desktop_firefox";
    HREFTXT = "You using FF on desktop, GO TO PLAY NOW!";
  }

  //
  if (navOpera && TYPEOFANDROID == 0 && !mobile) {
    browser_name = "desktop_opera";
    HREFTXT = "You using  OPERA browser on desktop, GO TO PLAY NOW!";
  }
}
///////////////////////////////////
//
...
function MakeSinValue(NAME, min_, max_, step, interval_) {
  ...
  TIMER_001 = setInterval(function() {
    if (CHECK_ == 0) {
      ...
      if (window["PERIODA" + NAME] > max_) {
        CHECK_ = 1;
      }
    } else if (CHECK_ == 1) {
      window["PERIODA" + NAME] = window["PERIODA" + NAME] - step;
      if (window["PERIODA" + NAME] < min_) {
        CHECK_ = 0;
      }
    } else {
    }
