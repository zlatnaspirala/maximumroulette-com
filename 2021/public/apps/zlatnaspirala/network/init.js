...
function login() {
  ...
  try {
    var lRes = lWSC.logon(
      lURL,
      jws.DEMO_ROOT_LOGINNAME,
      jws.DEMO_ROOT_PASSWORD,
      {
        // OnOpen callback
        ...
        OnMessage: function(aEvent, aToken) {
          ...
          if (aToken.type == "data") {
            if (aToken.publisher != lWSC.getId()) {
              //eliminate your's data only other
              //	console.log("TIP PODATKA DATA");
              var buf = Net_id.indexOf(aToken.publisher);
              if (buf == -1) {
                // Multiplayer buffer data
                if (aToken.publisher != null) {
                  ...
                  if (aToken.map.avatar === "undefined") {
                  } else {
                    try {
                      ...
                      CREATE_TEXTURE(
                        "T" + Net_id.indexOf(aToken.publisher).toString(),
                        localStorage.getItem("PROFILE_AVATAR")
                      );
                    } catch (e) {
                      console.log(e + "NEBI TREBALO DA SE DESI");
                    }
                  }
                ...
                //replace new data about network player

                Net_x.splice(
                  Net_id.indexOf(aToken.publisher),
                  1,
                  aToken.map.position[0]
                );
                Net_y.splice(
                  Net_id.indexOf(aToken.publisher),
                  1,
                  aToken.map.position[1]
                );
                Net_z.splice(
                  Net_id.indexOf(aToken.publisher),
                  1,
                  aToken.map.position[2]
                );

                if (aToken.map.avatar === "undefined") {
                } else {
                  try {
                    netPlayer_avatar.splice(
                      Net_id.indexOf(aToken.publisher),
                      1,
                      aToken.map.avatar[0]
                    );
              ...
              if (aToken.data != "") {
                ...
                if (MAIN.browser.name == "android_tab_firefox") {
                  getE("CHAT_DIV").innerHTML +=
                    '<div style="width:97%;height:auto;background-color:yellow;border-radius:14px;color:black;padding-left:10px;padding-top:4px;" >' +
                    DATA_CHAT +
                    "</div>";
...
function createChannel() {
  ...
  var lRes = lWSC.channelCreate(lChannelId, lChannelName, {
...
function removeChannel() {
  ...
  var lRes = lWSC.channelRemove(lChannelId, {
    accessKey: lAccessKey,
    secretKey: lSecretKey,
  });
  log(lWSC.resultToString(lRes));
