...
/* globals CANNON THREE Detector SmoothieChart TimeSeries app Stats dat*/
...
CANNON.Demo = function(options) {
  ...
  var view1Elem = document.querySelector("#view1");
  var view2Elem = document.querySelector("#view2");
  var canvas = null;
  ...
  function init() {
    ...
    if (typeof document.addEventListener === "undefined" || hidden === undefined) {
      console.log("This requires a browser supports the Page Visibility API.");
    } else {
    ...
    camera2 = new THREE.PerspectiveCamera( 22, 0.5 * SCREEN_WIDTH / SCREEN_HEIGHT,  NEAR, FAR);
    window.camera2 = camera2;
    app.camera2 = camera2;
    camera2.position.set( 0, 35, 0);
    camera2.lookAt(0, 1, 0);

    camera.position.set(-66.034239263301494, 85, -17.46855002841894);
    camera.up.set(1.148224077301623, 5.62947873437256, 0);
    ...
    // RENDERER
    renderer = new THREE.WebGLRenderer({ canvas: canvas, clearColor: 0x000000, clearAlpha: 1, antialias: true });
    ...
    info.innerHTML = "Ultimate Roulette 1.0.0 maximumroulette.com 2020";
    ...
    if(app.options.threejsGui == true && window.dat != undefined) {
      gui = new dat.GUI();
    ...
    controls.minPolarAngle = 1.5 * Math.PI / 12; // radians
    controls.maxPolarAngle = 1.3 * Math.PI / 3;
  ...
  function animate() {
    ...
    if (controls.enabled) {
      ...
      for(var i = 0; i < app.container.balls.length; i++) {
        app.updater.ballMeshes[i].position.copy(app.container.balls[i].position);
        app.updater.ballMeshes[i].quaternion.copy(app.container.balls[i].quaternion);
      }

      // Update box positions
      // eslint-disable-next-line no-redeclare
      for(var i = 0;i < app.container.boxes.length;i++) {
        app.updater.boxMeshes[i].position.copy(app.container.boxes[i].position);
        app.updater.boxMeshes[i].quaternion.copy(app.container.boxes[i].quaternion);
      }

      // Update wheel
      // eslint-disable-next-line no-redeclare
      if (app.container.wheelShapes.length > 0) {
      ...
      if (app.wheelRoll && app.gamePlay.spining) {
        app.wheelRoll.quaternion.setFromAxisAngle(new CANNON.Vec3(0,1,0), -app.getWheelSpinSpeed() * Math.PI/32);
        app.wheelRoll2.quaternion.setFromAxisAngle(new CANNON.Vec3(0,1,0), 93.5 - app.getWheelSpinSpeed() * Math.PI/32);
        app.centerRoll.quaternion.setFromAxisAngle(new CANNON.Vec3(0,1,0), 93.5 - app.getWheelSpinSpeed() * Math.PI/32);
      }
      ...
      if (scene.children.length > 20 && app.groupGamesIndicators != null) {
        ...
        if (intersects.length > 0) {
          ...
          if (app.INTERSECTED != intersects[0].object) {
            ...
            if ( app.INTERSECTED ) {
              app.INTERSECTED.material.opacity = app.INTERSECTED.currentOpacity;
            }

            app.INTERSECTED = intersects[0].object;
            app.INTERSECTED.currentOpacity = app.INTERSECTED.material.opacity;
            app.INTERSECTED.material.opacity = app.opacityForHoveredField;
      ...
      if (scene.children.length > 20 && app.groupControlsIndicators != null) {
        ...
        if (intersects.length > 0) {
          ...
          if (app.INTERSECTED_BTN != intersects[0].object) {
            ...
            // console.log(" INTER BTN---> " + intersects[0].object.position.y);
            if ( app.INTERSECTED_BTN ) {
              app.INTERSECTED_BTN.material[2].opacity = app.INTERSECTED_BTN.currentOpacity;
            }
            app.INTERSECTED_BTN = intersects[0].object;
            app.INTERSECTED_BTN.currentOpacity = app.INTERSECTED_BTN.material[2].opacity;
            app.INTERSECTED_BTN.material[2].opacity = app.opacityForHoveredField;
  ...
  function render() {
    ...
    var aspect = setScissorForElement(view1Elem);
...
CANNON.Demo.prototype.addVisual = function(body, mat, connectChip) {
  var s = this.settings;
  // What geometry should be used?
  var mesh;
  if(body instanceof CANNON.Body) {
    mesh = this.shape2mesh(body, mat);
  }
  if(mesh) {
    // Add body
    ...
    if (typeof connectChip !== "undefined") {
      app.container.boxes.push(body);
      app.updater.boxMeshes.push(connectChip);
      app.gamePlay.memoChips.push(body);
    }
...
CANNON.Demo.prototype.shape2mesh = function(body, mat) {
  ...
  for (var l = 0; l < body.shapes.length; l++) {
    ...
    switch(shape.type) {
      ...
      case CANNON.Shape.types.PLANE:
        ...
        var ground = null;
        if (typeof mat !== "undefined" && typeof mat.length == "undefined") {
      ...
      case CANNON.Shape.types.BOX:
        ...
        } else if (typeof mat !== "undefined" && typeof mat.length !== "undefined") {
          mesh = new THREE.Mesh(box_geometry, mat[l]);
        } else {
